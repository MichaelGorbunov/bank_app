...
def card_number_generator(start:int, stop:int)-> str |None:
...
def filter_by_currency(transactions:list[dict],currency:str = "USD")->int:
    filtered_transac = (filter(lambda x: x["operationAmount"]["currency"]["name"] == "USD", transactions))
    return filtered_transac


# filtered_people = list(filter(lambda x: x["operationAmount"]["currency"]["name"] == "USD", transactions))
# print(filtered_people)  # Output: [{'name': 'Bob', 'age': 30}, {'name': 'David', 'age': 30}]
...
#         n.append(operation["description"])
#     return n
